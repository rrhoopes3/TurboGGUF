...
@pytest.mark.skipif(sys.platform != "win32", reason="Windows-specific executable naming")
def test_find_llama_cpp_tools_accepts_repo_source_with_parent_binaries(tmp_path):
    """Common dev layout: source checkout in repo/, pre-built binaries in bin/.

    We warn (printed at the call site) but don't refuse — it's a normal
    way to keep a llama.cpp source checkout alongside a matching release
    binary set.
    """
    root = tmp_path / "llama.cpp"
    converter = _touch(root / "repo" / "convert_hf_to_gguf.py")
